...
def resolve_env_vars(value: str) -> str:
    ...
    Example:
        ...
        >>> resolve_env_vars("Value is ${MY_VAR}")
        'Value is hello'
        >>> resolve_env_vars("Value is ${UNSET:-default}")
        'Value is default'
    """
...
def resolve_env_vars_recursive(data: Any) -> Any:
    ...
    Traverses dictionaries and lists, resolving environment variables
    in all string values.

    Args:
        data: Data structure (dict, list, or scalar value)

    Returns:
        Data structure with environment variables resolved

    Example:
        >>> os.environ["API_KEY"] = "secret123"
        >>> resolve_env_vars_recursive({"key": "${API_KEY}"})
        {'key': 'secret123'}
    """
    if isinstance(data, dict):
        return {key: resolve_env_vars_recursive(val) for key, val in data.items()}
...
def load_deployment_config(agent_config_path: str | Path) -> "DeploymentConfig":
    """Load and validate deployment configuration from an agent config file.

    Loads the agent YAML file, extracts the deployment section,
    resolves environment variables, and validates against the
    Pydantic model.
    ...
    Raises:
        ConfigError: If the file doesn't exist, has no deployment section,
                    or fails validation

    Example:
        >>> config = load_deployment_config("agent.yaml")
    ...
    if not path.exists():
        raise ConfigError(
            field="agent_config_path",
            message=f"Agent configuration file not found: {path}",
        )
    ...
    if not isinstance(raw_config, dict):
        raise ConfigError(
            field="agent_config",
            message="Agent configuration must be a YAML dictionary",
        )

    if "deployment" not in raw_config:
        raise ConfigError(
            field="deployment",
            message="No 'deployment' section found in agent configuration",
        )

    # Resolve environment variables in deployment section
    deployment_data = resolve_env_vars_recursive(raw_config["deployment"])

    try:
        return DeploymentConfig(**deployment_data)
    except Exception as e:
        raise ConfigError(
            field="deployment",
            message=f"Invalid deployment configuration: {e}",
        ) from e


def get_agent_name(agent_config_path: str | Path) -> str:
    """Extract the agent name from an agent configuration file.
    ...
    Raises:
        ConfigError: If the file doesn't exist or has no name field

    Example:
        >>> name = get_agent_name("agent.yaml")
        >>> print(name)
        'my-agent'
    """
    from holodeck.lib.errors import ConfigError
    ...
    if not name:
        raise ConfigError(
            field="name",
            message="Agent configuration must have a 'name' field",
        )
