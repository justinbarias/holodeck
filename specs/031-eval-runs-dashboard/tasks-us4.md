...
- [x] T367 [US4] Open the prototype: `mcp__claude-in-chrome__tabs_context_mcp` → `tabs_create_mcp` with `url="http://localhost:8000/Evaluation%20Dashboard.html"` → wait for load → `take_screenshot`. Save as `specs/031-eval-runs-dashboard/visual-baselines/prototype-summary.png`
- [x] T368 [US4] Open the Dash app: `tabs_create_mcp` with `url="http://127.0.0.1:8501/?tab=summary"` → wait for the main chart (`find` on `#chart-pass-rate`) → `take_screenshot`. Save as `specs/031-eval-runs-dashboard/visual-baselines/dash-summary-v1.png`
