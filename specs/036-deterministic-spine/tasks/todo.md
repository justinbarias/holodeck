...
- [ ] **CHECKPOINT 1** — partially met: `make ci` clean ✔ (5298 unit tests passing, 4 skipped; format, lint, type-check, security, pre-commit all clean) · single edge→policy workflow runs via CLI ✔ (`sample/pbas-points/`, live Claude call) · **human review before Phase 2 — outstanding**
