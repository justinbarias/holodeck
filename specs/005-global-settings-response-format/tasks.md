...
- [ ] T023 [P] Integration test: End-to-end config loading and agent initialization in `tests/integration/test_config_end_to_end.py`
...
- [ ] T024 Code quality: Format, lint, type-check, security scan with `make format`, `make lint`, `make type-check`, `make security` (all must pass)
