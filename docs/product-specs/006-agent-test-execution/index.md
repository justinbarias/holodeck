# Agent Test Execution — specification

Feature: `006-agent-test-execution`. Status and task evidence: [feature inventory](../inventory.md).

## Documents

- [checklists/requirements.md](checklists/requirements.md)
- [contracts/cli-api.md](contracts/cli-api.md)
- [quickstart.md](quickstart.md)
- [spec.md](spec.md)

## Related records

- [Design records](../../design-docs/006-agent-test-execution/index.md)
- [Execution plans](../../exec-plans/completed/006-agent-test-execution/index.md)
- [Execution-plan lifecycle](../../PLANS.md)
