...
> `docs/product-specs/040-holodeck-temporal/spec.md`, which records the decisions that produced the
> pivot.
...
**Related**: `docs/ideas/deterministic-spine.md` (idea one-pager), `docs/product-specs/029-subagent-orchestration`, `docs/product-specs/018-otel-observability`, `docs/product-specs/022-otel-genai-semconv`, `docs/product-specs/006-agent-test-execution`
**Successor**: `docs/product-specs/039-policy-generator` — AI-drafted decision tables and DRDs from policy documents. Specced in parallel so its schema needs land as additions here while this schema is still soft; **built only after this spec's MVP ships**.
...
- **AI-drafted decision tables and DRDs** — `docs/product-specs/039-policy-generator`. This spec ships only the artifacts that make a drafted table safe to run: the `provenance` block and the review gate (FR-029…FR-032).
