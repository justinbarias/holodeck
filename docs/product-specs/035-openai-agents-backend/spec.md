...
| Cross-backend P3 Envoy profile | [H-022](../../exec-plans/tech-debt-tracker.md#h-022) |
...
- **FR-090 (P3)**: **Deferred proposal**, outside 035 completion; [D08](#d08), [H-022](../../exec-plans/tech-debt-tracker.md#h-022). Future requirement: `deployment.security_profile: hardened` MUST work identically: Envoy sidecar holds credentials, agent container has none, domain allowlist derived from YAML.
- **FR-091 (P3)**: **Deferred proposal**, outside 035 completion; [D08](#d08), [H-022](../../exec-plans/tech-debt-tracker.md#h-022). Future requirement: Allowlist derivation for `provider: openai` MUST include `api.openai.com`, the embedding-provider endpoint, and each MCP HTTP/SSE endpoint. For `provider: azure_openai`, the allowlist MUST additionally include the resolved Azure resource hostname (`<resource>.openai.azure.com`) extracted from `AZURE_OPENAI_ENDPOINT`.
- **FR-092 (P3)**: **Deferred proposal**, outside 035 completion; [D08](#d08), [H-022](../../exec-plans/tech-debt-tracker.md#h-022). Future requirement: For `provider: openai`, the agent container MUST be configured with `OPENAI_BASE_URL=http://localhost:<envoy-port>` and `HTTPS_PROXY=http://localhost:<envoy-port>`. For `provider: azure_openai`, `AZURE_OPENAI_ENDPOINT` MUST be rewritten to the localhost sidecar URL; the original Azure hostname is reachable only from the sidecar.
- **FR-093 (P3)**: **Deferred proposal**, outside 035 completion; [D08](#d08), [H-022](../../exec-plans/tech-debt-tracker.md#h-022). Future requirement: The `openai_agents` backend MUST refuse to start in hardened profile if any credential-bearing env var is set on the agent container (the operator has half-migrated).
...
- **SC-010**: **Deferred proposal**, not a 035 completion gate; [D08](#d08), [H-022](../../exec-plans/tech-debt-tracker.md#h-022). Future acceptance: `security_profile: hardened` deploys two containers; the agent container env has zero credential-bearing vars; calls to non-allowlisted domains are rejected by Envoy.
...
**P3 and sandbox remain explicit exclusions.** Full-plan Decisions 2/3 defer net-new shared Envoy infrastructure and security-sensitive sandbox lifecycle work. FR-090–093/SC-010 go to H-022; FR-094–099/SC-012/013 go to H-011. Their retained MUST wording is future proposal text, not a claim of supported YAML or a current completion gate. P1a/P1b/P2a/P2b remain required. Spec-034 references do not imply shared P3 infrastructure already exists.
...
**LiteLLM inference contracts: dimensions reach the provider, provider errors surface, semantic conventions are not pinned.** The tool initializer builds one shared `LiteLLMEmbeddingService` per agent (one embedding model, provider-native size) and a dedicated service for every tool that sets `embedding_dimensions`, so the override is sent to the provider as `dimensions` rather than only compared against the returned vector length (allow-listed for Azure deployment aliases, whose names carry no model marker); tools with different overrides call the provider with different sizes, and the tool-side length check remains the final guard. `LiteLLMEmbeddingService.generate_embeddings` is the provider-error boundary: any provider or decoding failure is re-raised as `EmbeddingServiceError` with the original exception chained, and `VectorStoreTool` / `HierarchicalDocumentTool` propagate it instead of substituting placeholder vectors (ingest failures abort tool initialization as `ToolInitializerError`; query failures surface as the tool error; placeholders remain only when no service is injected). HoloDeck does not set `OTEL_SEMCONV_STABILITY_OPT_IN`: the documented span shape is LiteLLM's default (`litellm_request` spans, operation on `llm.request.type`, provider on `gen_ai.system`), operators may opt into `gen_ai_latest_experimental` (`embeddings <model>` / `chat <model>`, `gen_ai.provider.name`), and the model, token-usage, and content attribute names are identical in both shapes so redaction and `NO_CONTENT` hold either way. `RedactingSpanProcessor` also scrubs LiteLLM's raw-response attribute, `error.*` failure attributes, and span-event attributes (the OTel `exception` event), which LiteLLM writes regardless of content capture. Exporter-level tests over the real LiteLLM entry points are in `tests/unit/lib/observability/test_litellm_spans.py`; live provider evidence remains T10.

<a id="d16"></a>

### D16

**Subagents and skills are SDK handoff targets built from the parent's tool surface; handoff events reuse the Claude `ToolEvent` contract.** `openai.agents.<name>` (`OpenAISubagentSpec`: `description`, `prompt`/`prompt_file`, `tools`, `model`, `skip_recommended_prefix`) and `type: skill` tools (`SkillTool`, net-new in `ToolUnion`: inline `instructions` or a `path` whose `SKILL.md` frontmatter supplies `name`/`description` and whose body supplies the instructions, plus YAML-only `allowed_tools`) both become SDK `Agent` objects on the parent's `handoffs` (`openai_agents_subagents.py`). Tool grants are resolved against the parent's *built* SDK tools and MCP servers, indexed by the config names written under the parent's `tools:` (the same portable naming `disallowed_tools` uses), so a disallowed parent tool can never be granted downstream; `tools: null` inherits every parent tool and server, an explicit list restricts, an empty list grants nothing, and an unknown name fails load as `ConfigError`. Skills get no tools unless `allowed_tools` names them, and `allowed_tools` is cross-validated against the agent's non-skill tools at config load. `model: inherit` (or omitted) reuses the parent's model object including any fallback wrapper; an explicit identifier is passed through for OpenAI or wrapped as an Azure deployment on the parent endpoint, without a fallback wrapper; `sonnet|opus|haiku` fail config validation. `RECOMMENDED_PROMPT_PREFIX` is prepended exactly once to subagent instructions unless `skip_recommended_prefix: true`; skill instructions are used verbatim so inline and file-based forms produce identical agents. A skill name colliding with an `openai.agents` key fails load. `claude.setting_sources` is accepted and, once per backend load, logs `setting_sources is a Claude-only concept; ignored on openai.` with no ambient discovery (H-020). Every target also receives the parent's `output_type` (so a run that ends inside a handoff still honours `response_format`) and the parent's `ModelSettings` for `inherit`/skills or settings rebuilt for an explicit model. Two parent tools that would surface under one SDK tool name (vectorstore `kb` and function `kb_search`) and two targets that normalise to one `transfer_to_` tool fail load, so grants and disallow lists are never ambiguous; `load_agent_yaml` sets `agent_base_dir` before validation so `prompt_file` and skill `path` resolve against the loaded file's directory. Events (`openai_agents_events.py`): `ToolCallItem`/`HandoffCallItem` → `start` (+`parent_link` under an active handoff), `ToolCallOutputItem` → `end`, `HandoffOutputItem` makes the handoff the active parent (its `end` is deferred to run completion so the chat panel keeps nesting the subagent's activity under it), `AgentUpdatedStreamEvent` after a handoff → `subagent_message` announcing the active agent (emitted from the handoff output item on the non-streaming path, which has no agent-updated event), subagent `MessageOutputItem` → `subagent_message` snapshot, `ReasoningItem` summaries → `thinking`; at run end every open handoff gets `end`, and a run that fails mid-stream closes every open tool call and handoff with `error`. The session exposes a bounded `tool_events` queue (1000 entries, newest dropped on overflow), feeding the existing chat panel and AG-UI bridge unchanged; a local tool failure surfaces as an `end` event carrying the SDK's error text rather than an `error` event. Handoff calls are also recorded in `ExecutionResult.tool_calls`/`tool_results` (`handoff:<target>`). Bounded fixture evidence drives the real SDK `Runner` with a scripted model (`tests/unit/lib/backends/test_openai_agents_events.py`); a live handoff remains T10.

<a id="d17"></a>

### D17

**Hosted tools are typed `type: hosted` entries built into the SDK's server-side tools; the safety gate runs after permission filtering and the Claude backend rejects them.** A hosted entry carries a HoloDeck config `name` (the same identifier `disallowed_tools` and subagent `tools` lists use for every tool type), a `tool` discriminator naming the SDK class, and a per-class validated `params` model (`extra: forbid`) in `models/tool.py`: `WebSearchTool` (`user_location`, `allowed_domains`, `search_context_size`, `external_web_access`), `FileSearchTool` (`vector_store_ids` required, `max_num_results`, `include_search_results`, `ranking_options`, pass-through `filters`), `CodeInterpreterTool` (`container` required: id or `{type: auto, file_ids, memory_limit, network_policy}`), `ImageGenerationTool` (all optional), and `HostedMCPTool` (`server_label` plus exactly one of `server_url`/`connector_id`; `authorization`/`headers` are `${VAR}`-substituted at build; `require_approval` is a `Literal["never"]` because interactive approval is H-013 and an unsupported gate must fail closed rather than run). `ComputerTool` is a union member whose validator always fails with the H-012 message, so it is rejected at config load on every backend and regardless of the unsafe opt-in. `openai_agents_tool_adapters.build_hosted_tool` constructs the SDK dataclasses, building the nested `CodeInterpreter`/`ImageGeneration`/`Mcp` `tool_config` mappings from the params and forwarding only fields set in YAML. Hosted entries surface under the SDK's fixed tool names (`web_search`, `file_search`, `code_interpreter`, `image_generation`, `hosted_mcp`), so two entries of one class, or a function tool named like one, fail the existing SDK-name collision check; subagents grant them by config name through the parent tool surface like any other tool. Ordering (FR-083/FR-034): `disallowed_tools` filtering runs first, so a disallowed code interpreter is never built and needs no acknowledgement; otherwise a declared `CodeInterpreterTool` without `openai.i_understand_this_is_unsafe: true` is reported by `validate_openai_agents` in the same pass as credential and allow/deny conflicts (FR-110) and, defensively, by the factory. `validate_no_hosted_tools` fails Claude-backend initialization naming every hosted entry. Azure (D06): hosted entries load; a run-time failure keeps the SDK error verbatim and appends a hint naming the declared hosted classes when the message points at tool support. Hosted call items (`web_search_call`, `file_search_call`, `code_interpreter_call`, `image_generation_call`, `mcp_call`) never get a `ToolCallOutputItem`, so `openai_agents_events.hosted_call_for` reconstructs name, arguments, and a status/summary response from the call item; the event stream emits `start` and `end` together and `ExecutionResult` pairs the call and result (hosted MCP calls use the remote tool name with `server_label` in the arguments; image bytes are omitted). Per-factory tests over the real SDK classes are in `tests/unit/lib/backends/test_openai_agents_hosted_tools.py` and `tests/unit/models/test_hosted_tool.py`; live provider evidence remains T10.
...
Hooks and guardrails remain implementation work; do not treat forecast filenames as delivered modules.
