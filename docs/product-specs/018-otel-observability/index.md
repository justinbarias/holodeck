# OTel Observability — specification

Feature: `018-otel-observability`. Status and task evidence: [feature inventory](../inventory.md).

## Documents

- [checklists/requirements.md](checklists/requirements.md)
- [contracts/observability-api.md](contracts/observability-api.md)
- [contracts/observability-config.schema.json](contracts/observability-config.schema.json)
- [quickstart.md](quickstart.md)
- [spec.md](spec.md)

## Related records

- [Design records](../../design-docs/018-otel-observability/index.md)
- [Execution plans](../../exec-plans/active/018-otel-observability/index.md)
- [Execution-plan lifecycle](../../PLANS.md)
