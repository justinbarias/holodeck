...
| <a id="h-009"></a>H-009 | [LiteLLM acceptance gaps](active/035-openai-agents-backend/plan-litellm-embeddings-contextgen.md): dimension plumbing, error contract, telemetry evidence, and provider documentation. | Retrieval and observability maintainers | Resolve each documented contract difference and supply focused and provider acceptance evidence. | Open; focused contracts and documentation closed by 035 T2 (2026-09-06), live provider evidence pending T10 |
| <a id="h-010"></a>H-010 | Structured `vectorstore` sources on `provider: qdrant` whose `id_field` values are not UUIDs or integers are stored under UUIDv5 hashes ([vectorstore_tool.py](../../src/holodeck/tools/vectorstore_tool.py) `_coerce_record_id`), so `StructuredQueryResult.id` carries the hash; the original key survives only when listed in `meta_fields`. A warning is logged once per tool. | RAG maintainers | Persist the original record key on the structured record schema (ask-first schema change) and return it through `StructuredQueryResult.id`, with a regression test on Qdrant. | Open |
| <a id="h-023"></a>H-023 | The bundled `max_budget_usd` price table ([openai_agents_cost.py](../../src/holodeck/lib/backends/openai_agents_cost.py), dated 2026-06-13) has no entry for `gpt-5.4` (or other post-`gpt-5` models), so budget enforcement is silently disabled with a warning on those deployments; observed on the E11 Azure run. | Backend maintainers | Refresh the table from OpenAI list pricing with a new table date and add a test that every deployment name used by the tracked samples resolves to a price. | Open (2026-09-06) |
