...
- [x] **T16** Live worker-command integration test: real `holodeck worker` subprocess against `WorkflowEnvironment.start_local` (real dev server, no mocks), workflow executes end to end, SIGINT shuts down gracefully (M)

### Checkpoint 3
- [x] `holodeck worker --help` works without the extra; worker starts from fixture config (mocked client); live worker e2e green
...
- [x] **T10** Integration AC-1/AC-2/AC-3 against `start_local`, mocked backend (CI-runnable; live counterpart = T13 smoke) (M)
- [x] **T11** Integration AC-4 (worker subprocess e2e) + AC-5 (replay, zero LLM calls) + Worker-init sandbox backstop (M)
- [x] **T12** OTel AC-6: span parity with `holodeck test`; GenAI spans nest under activity span (M)
- [x] **T13** Live smoke `@slow`, `sample/temporal-hardship/` demo, `docs/product-specs/inventory.md` row (S)
