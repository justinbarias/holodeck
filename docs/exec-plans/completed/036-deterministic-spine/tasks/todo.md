...
> the archive banner in `../spec.md` and the successor `docs/product-specs/040-holodeck-temporal/spec.md`.
...
- **039 policy generator** — AI-drafted tables + DRD from policy documents.
  Spec written in parallel (`docs/product-specs/039-policy-generator/`) so its schema needs
  land here as additions while this schema is soft. **Built only after the MVP.**
  Corpus pinned: see `docs/product-specs/039-policy-generator/corpus-manifest.md`.
