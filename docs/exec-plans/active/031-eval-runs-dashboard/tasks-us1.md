...
**Spec**: [spec.md](../../../product-specs/031-eval-runs-dashboard/spec.md) — User Story 1 (P1)
**Plan**: [plan.md](plan.md)
**Data model**: [data-model.md](../../../design-docs/031-eval-runs-dashboard/data-model.md)
**Research**: [research.md](../../../design-docs/031-eval-runs-dashboard/research.md)
**Contract**: [contracts/cli.md](../../../product-specs/031-eval-runs-dashboard/contracts/cli.md)
...
**Dashboard readiness**: the additive `TestResult` fields are not decorative — they are the minimum runtime data required for the dashboard's Summary breakdowns (kind), Explorer tool-call panels (`tool_invocations`), and Compare view cost row (`token_usage`). See [data-model.md](../../../design-docs/031-eval-runs-dashboard/data-model.md) "Dashboard Field Projections" and [research.md](../../../design-docs/031-eval-runs-dashboard/research.md) R8 (pairing) / R9 (token capture).
...
- [x] T010a [P] [US1] (TDD) `tests/unit/models/test_metric_result_kind.py`: assert `MetricResult.model_fields["kind"].annotation` is `Literal["standard","rag","geval"]`; assert `kind` is required (no default); round-trip via `model_dump_json()` / `model_validate_json()` preserves it
- [x] T010b [US1] Extend `MetricResult` in `src/holodeck/models/test_result.py:35` with `kind: Literal["standard","rag","geval"] = Field(..., description="Metric family — distinguishes standard NLP / RAG / GEval")`. The discriminator already exists on the evaluation CONFIG side (`models/evaluation.py:43,149,250`) as `type: Literal["standard"|"geval"|"rag"]`; T010b propagates it to runtime results
- [x] T010c [US1] **Code reality-check**: the task text said to patch evaluator writers, but `MetricResult(...)` is constructed in exactly **two sites in `executor.py` (success path ~line 815, error path ~line 842)** — evaluators return `dict` and the executor wraps. Added helper `_metric_kind(metric_config)` that maps the already-existing `metric_config.type` discriminator (`"standard"|"geval"|"rag"`) to `kind`. Patched both sites.
- [x] T010d [US1] Back-compat: added `src/holodeck/lib/eval_run/legacy.py` exporting `infer_metric_kind(metric_name)`. Names in `{bleu, rouge, meteor, exact_match, f1_score}` → `"standard"`; values in `RAGMetricType` enum → `"rag"`; everything else → `"geval"`. WARNING logged on every call. Tests in `tests/unit/lib/eval_run/test_legacy_kind_inference.py`.
...
> **Naming decision (2026-04-18 revisit)**: the persisted model is `ToolInvocation`, **not** `ToolEvent`. Two `ToolEvent` classes already exist (`models/tool_event.py` — streaming UI events; `lib/backends/base.py::ToolEvent` — backend hook events). A third `ToolEvent` would be confusing. `ToolInvocation` unambiguously means "one completed call+result pair on disk". See [research.md](../../../design-docs/031-eval-runs-dashboard/research.md) R8 for the per-backend pairing contract.

- [x] T010e [P] [US1] (TDD) `tests/unit/models/test_tool_invocation.py`: assert `ToolInvocation` has fields `name: str`, `args: dict[str, Any]` (default `{}`), `result: Any` (default `None`), `bytes: int` (ge=0), `duration_ms: int | None` (default `None`), `error: str | None` (default `None`), with `extra="forbid"`. Assert `TestResult.tool_invocations: list[ToolInvocation]` exists with `default_factory=list`; existing `tool_calls: list[str]` remains for back-compat. Round-trip via `model_dump_json()` / `model_validate_json()` preserves all fields.
- [x] T010f [US1] Added `class ToolInvocation(BaseModel)` co-located in `src/holodeck/models/test_result.py`. Added `tool_invocations: list[ToolInvocation] = Field(default_factory=list, ...)` on `TestResult`. `models/tool_event.py` left untouched to avoid naming collision with the two existing `ToolEvent` classes.
- [x] T010g [P] [US1] (TDD) `tests/unit/lib/test_runner/test_tool_invocation_builder.py`: all five cases covered (SK index pair, SK length mismatch, Claude id pair, Claude unmatched id, orphan result logs WARNING, JSON-safe coercion of `datetime` results).
- [x] T010h [US1] Implemented `src/holodeck/lib/test_runner/tool_invocation_builder.py` exporting `pair_tool_calls(tool_calls, tool_results, backend_kind)`. **Signature note**: the task draft said `(exec_result, backend_kind)` but I went with the already-paired lists `(tool_calls, tool_results, backend_kind)` — easier to unit-test without a fake `ExecutionResult`. Executor passes `raw_tool_calls=list(exec_result.tool_calls)` and `tool_results=exec_result.tool_results` at the call site. **Correlation key**: Claude's dict key is `"call_id"` (populated from `ToolUseBlock.id`/`ToolResultBlock.tool_use_id` by `claude_backend.py:162-177`), not `"tool_use_id"` as some task text said. JSON-safety via `json.loads(json.dumps(result, default=str))`.
- [x] T010i [US1] Wired `pair_tool_calls` into `executor.py`. Added `raw_tool_calls: list[dict[str, Any]]` alongside the existing `tool_calls: list[str]` so both shapes coexist. Backend-kind detection is `type(backend).__name__ == "ClaudeBackend"` — keeps the `AgentBackend` protocol untouched and avoids an import cycle through `BackendSelector`. Wrapped the `pair_tool_calls` call in `try/except TypeError` to tolerate mock-based unit tests that pass non-list sentinels.
- [x] T010j [US1] Updated `TestResult` docstring to state "Readers prefer `tool_invocations` (structured) when non-empty; `tool_calls: list[str]` (name-only) remains for back-compat with legacy runs predating Phase 2b."
...
- [x] T010k [P] [US1] (TDD) `tests/unit/models/test_test_result_token_usage.py`: covers default-`None`, round-trip when populated, and legacy-JSON loads as `None`.
- [x] T010l [US1] Added `token_usage: TokenUsage | None = Field(default=None, ...)` to `TestResult`; imported `TokenUsage`.
- [x] T010m [US1] Extracted `token_usage: TokenUsage | None = None` to scope outside the `if self._backend / elif self.agent_factory` branches; populated from `exec_result.token_usage` (new path) or `legacy_result.token_usage` (old path). Guarded the assignment with `isinstance(raw, TokenUsage)` to tolerate mock-based unit tests. Passed to `TestResult(...)` at the construction site.
...
**Checkpoint for Phase 2b**: ✅ All unit tests green (4173 passed, 1 skipped; +46 new tests for Phase 2b). `make lint` passes. `make type-check` has no new errors from Phase 2b (pre-existing errors from the Phase 2 SecretStr migration remain and are out of scope for this phase). Manual `holodeck test` against a persisted `EvalRun` is deferred until Phase 3 wires the CLI writer (US1 T030).
