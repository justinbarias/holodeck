...
and error handling with exponential backoff. The LLM call is backed by
``litellm.acompletion``, which is mocked throughout.
"""
...
TEST_SPEC = LiteLLMModelSpec(model="gpt-4o-mini", api_key="test-key")
...
def make_completion_response(content: str) -> MagicMock:
    """Build a litellm-style completion response with one choice."""
    message = MagicMock()
...
def patch_acompletion(
    ...
    """Patch litellm.acompletion with a side-effect or a fixed response."""
    if side_effect is not None:
        return patch("litellm.acompletion", new=AsyncMock(side_effect=side_effect))
    return patch(
        "litellm.acompletion",
        new=AsyncMock(return_value=make_completion_response(content or "")),
    )
...
class TestLLMContextGeneratorInit:
    ...
    def test_init_with_model_spec(self) -> None:
        """Test initialization with a LiteLLM model spec."""
        generator = LLMContextGenerator(model_spec=TEST_SPEC)
        assert generator._model_spec is TEST_SPEC
...
class TestGenerateContext:
    ...
    async def test_returns_context_string(self) -> None:
        ...
        generator = LLMContextGenerator(model_spec=TEST_SPEC)
        with patch_acompletion(content="This chunk describes the authentication flow."):
    ...
    @pytest.mark.asyncio
    async def test_calls_llm_with_formatted_prompt_and_spec(self) -> None:
        """Test that litellm is called with the prompt and spec kwargs."""
        generator = LLMContextGenerator(model_spec=TEST_SPEC)
        with patch(
            "litellm.acompletion",
            new=AsyncMock(return_value=make_completion_response("Context here")),
        ) as mock_completion:
        ...
        assert kwargs["max_tokens"] == generator._max_context_tokens
        messages = kwargs["messages"]
        assert messages[0]["role"] == "user"
        assert "Sample chunk" in messages[0]["content"]
        assert "Sample document" in messages[0]["content"]

    @pytest.mark.asyncio
    async def test_strips_whitespace_from_response(self) -> None:
        """Test that response whitespace is stripped."""
        generator = LLMContextGenerator(model_spec=TEST_SPEC)
        with patch_acompletion(content="  Context with whitespace  \n\n"):
            context = await generator.generate_context(
                chunk_text="Chunk",
    ...
    async def test_handles_empty_response(self) -> None:
        ...
        generator = LLMContextGenerator(model_spec=TEST_SPEC)
        with patch_acompletion(content=""):
            context = await generator.generate_context(
                chunk_text="Chunk",
                document_text="Document",
            )
...
class TestContextualizeChunk:
    ...
    async def test_prepends_context_to_content(self) -> None:
        ...
        with patch_acompletion(content="This chunk is about authentication."):
    ...
    async def test_format_is_context_newlines_content(self) -> None:
        ...
        with patch_acompletion(content="Context text"):
            result = await generator.contextualize_chunk(
                chunk=chunk,
                document_text="Document",
            )
...
class TestContextualizeBatch:
    ...
    async def test_processes_all_chunks(self) -> None:
        ...
        async def mock_completion(**kwargs: Any) -> MagicMock:
            nonlocal call_count
            call_count += 1
            return make_completion_response(f"Context {call_count}")

        generator = LLMContextGenerator(model_spec=TEST_SPEC)
        chunks = [create_test_chunk(chunk_index=i) for i in range(5)]

        with patch_acompletion(side_effect=mock_completion):
    ...
    async def test_respects_concurrency_limit(self) -> None:
        ...
        async def mock_completion(**kwargs: Any) -> MagicMock:
            ...
            concurrent_calls -= 1
            return make_completion_response("Context")
    ...
    async def test_maintains_chunk_order(self) -> None:
        ...
        async def mock_completion(**kwargs: Any) -> MagicMock:
            ...
            # Add small pseudo-random delay to shuffle completion order
            await asyncio.sleep(0.01 * (hash(prompt) % 5))
            call_order.append(len(call_order))
            return make_completion_response(f"Context {len(call_order)}")
...
class TestExponentialBackoff:
    ...
    async def test_retries_on_failure(self) -> None:
        ...
        async def mock_completion(**kwargs: Any) -> MagicMock:
            ...
            return make_completion_response("Success")
...
class TestDocumentTruncation:
    ...
    async def test_truncates_long_document(self) -> None:
        ...
        async def mock_completion(**kwargs: Any) -> MagicMock:
            ...
            received_prompt = kwargs["messages"][0]["content"]
            return make_completion_response("Context")
...
class TestProtocolConformance:
    ...
    def test_is_context_generator(self) -> None:
        ...
        gen = LLMContextGenerator(model_spec=TEST_SPEC)
