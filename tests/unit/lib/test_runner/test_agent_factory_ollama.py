...
class TestOllamaEmbeddingService:
    ...
    Note: Embedding service creation is now delegated to the shared
    ``holodeck.lib.tool_initializer`` module.  Mocks target the Ollama
    module via ``sys.modules`` dict-patch so the lazy import inside
    ``create_embedding_service()`` picks up the mock.
    """
    ...
    def test_register_embedding_service_ollama_local(
        ...
        with patch.dict(
            "sys.modules",
            {
                "semantic_kernel.connectors.ai.ollama": Mock(
                    OllamaTextEmbedding=mock_ollama_embedding
                )
            },
        ):
            factory = AgentFactory(agent_config)
    ...
    def test_embedding_service_ollama_package_missing(
        ...
        with (
            ...
            pytest.raises(AgentFactoryError) as exc_info,
        ):
            AgentFactory(agent_config)
